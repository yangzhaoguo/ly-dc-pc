<template>
  <div id="">
    ...
    <div class="content" ref="viewBox">
      <ul ref="chatContainer" v-if="list.length>0">
        <li class="flex justify-between list" v-for="(item , i) in list" :key="i">
          ...
          <div style="margin:auto 0" @click="goItem(item.distance_id)">
            <el-button
              v-if="item.distance_status===1||item.distance_status===2||item.distance_status===3||item.distance_status===4"
              type="primary">进行中
            </el-button>
            <el-button v-if="item.distance_status===5" type="warning">待支付</el-button>
            <el-button v-if="item.distance_status===6" type="success">待评价</el-button>
            <el-button v-if="item.distance_status===7" type="">已评价</el-button>
          </div>
        </li>
      </ul>
